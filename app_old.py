...
ns_report = api.namespace('report', description='รายงานการขอไปราชการ')
ns_kpi = api.namespace('kpi', description='ระบบพัฒนาทรัพยากรบุคคล')


### เข้าสู่ระบบ ###
@ns_login.route('/login', methods=['POST'])
class HRDLogin(Resource):
    def post(self):
        try:
            ...
            encode = jwt.encode({'data':[json_data[0]], 'iat': iat, 'exp': EXP}, 'secret', algorithm='HS256')
...
@ns_hrd.route('/checkDepart', methods=['GET'])
class updatePersonal(Resource):
    def get(self):
        try:
...
@ns_report.route('/person', methods=["post"])
...
def getDepartment(dep):
    #strDep = "%" + dep + "%"
    curs = mysql.connection.cursor()
    sql = """SELECT * FROM hrd.department WHERE dep_code_name = '%s'""" % (
        dep)
    curs.execute(sql)
    # แยกส่วนหัวของแถว
    row_headers = [x[0] for x in curs.description]
